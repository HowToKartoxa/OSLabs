...
BOOST_AUTO_TEST_CASE(employee_db_small_init)
{
	...
	{
		test_data.emplace_back(i, std::to_string(i), i);
	}
...
{
	std::vector<Employee> test_data;
	test_data.emplace_back(1, "test_employee", 10);
	test_data.emplace_back(1, "duplicate_test_employee", 8);
	EmployeeDB database("test_db", test_data);
...
BOOST_AUTO_TEST_CASE(employee_db_small_get)
{
	...
	Employee temp_employee;
	size_t locked_at;
	BOOST_TEST(database.WGetShared(3ull, temp_employee, locked_at) == true);
	BOOST_TEST(temp_employee.id == 3ull);
	int res = strcmp(temp_employee.name, "3");
	BOOST_TEST(res == 0);
	BOOST_TEST(temp_employee.hours == 3.0);
	BOOST_TEST(database.WGetShared(10ull, temp_employee, locked_at) == false);
}
...
BOOST_AUTO_TEST_CASE(employee_db_large_get)
{
	...
	Employee temp_employee;
	size_t locked_at;
	BOOST_TEST(database.WGetShared(1900ull, temp_employee, locked_at) == true);
	BOOST_TEST(temp_employee.id == 1900ull);
	int res = strcmp(temp_employee.name, "1900");
	BOOST_TEST(res == 0);
	BOOST_TEST(temp_employee.hours == 1900);
	BOOST_TEST(database.WGetShared(10000ull, temp_employee, locked_at) == false);
}
